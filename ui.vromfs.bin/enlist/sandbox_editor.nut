...
let {editorIsActive, editorFreeCam, initTemplatesGroups, initRISelect, proceedWithSavingUnsavedChanges, hideDebugButtons} = require("editor.nut")
...
let prefabsBtn = textButton(
    ...
    {hotkeys = [["L.Ctrl K | R.Ctrl K"]], boxStyle = {normal = {fillColor = Color(0,0,0,80)}},
     onHover = @(on) cursors.setTooltip(on && !toolboxShown.value ? "Entity prefabs (Ctrl+K)" : null)})
...
let function sandboxEditor(){
  ...
  return {
    size = flex()
    watch = [levelIsLoadedAndEditorIsActive, scenesOpened, hintShown, editorIsActive, editorFreeCam, levelLoaded, levelIsLoading, levelIsNotPresent, toolboxShown, showPointAction, namePointAction,
