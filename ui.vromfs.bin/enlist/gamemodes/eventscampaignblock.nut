...
let eventsCampaignBlock = watchElemState(@(sf){
  ...
  behavior = Behaviors.Button
  gap = smallPadding
  color = ((sf & S_HOVER) && campaignsCount.value > 1) || !hasChoosedCampaign.value
    ? Color(35, 35, 35, 25)
    : blurBgFillColor
  valign = ALIGN_CENTER
  ...
  padding = [0, localGap]
  children = [
    campaignsCount.value < 2 || hasChoosedCampaign.value ? null : mkText(loc("army/selectNew"), defTxtColor, fontSub)
      @(){
