...
let function join_session(data) {
  ...
  session.join(session_id, invitation_id, function(squad_id) {
    acceptSquadInvite(squad_id)
  })
}
...
let function onSessionInvitation(data) {
  log($"[PSEV] got invitation to {data?.session_id}: uinfo {userInfo?.value}, lstage {loginChain?.currentStage.value}")
...
let function onGameIntent(data) {
  log($"[PSEV] got game intent {data?.action}, session {data?.sessionId}, activity {data?.activityId}")
  if (data.sessionId != "") {
...
let function on_ps4_callback(data) {
  if (!data?.type) {
    log("[PSEV] Invalid data received: ", data)
    return
