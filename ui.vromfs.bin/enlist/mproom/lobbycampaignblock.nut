...
let header = @() {
  watch = isPrevCampaignNotAvailible
  halign = ALIGN_CENTER
  hplace = ALIGN_CENTER
  flow = FLOW_VERTICAL
  children = {
...
let campaignSelect = @() {
  watch = [roomCampaigns, campPerRow]
  hplace = ALIGN_CENTER
  vplace = ALIGN_CENTER
  flow = FLOW_VERTICAL
  halign = ALIGN_CENTER
  gap = gapCards
  children = [header].extend(mkRows(roomCampaigns.value, campPerRow.value))
...
let lobbyCampaignBlock = watchElemState(@(sf){
  rendObj = ROBJ_SOLID
  watch = [campaignsCount, hasPlayedCurSession]
...
let function closeCb() {
  ...
  isPrevCampaignNotAvailible(false)
}
...
isPrevCampaignNotAvailible.subscribe(@(v) v ? isOpened(true) : null)
