...
let mkLightCtorExt = @(lightCtor, nextStepDelay) function(box) {
  let { ctor = null, onClick = null, hotkey = null } = box
  if (ctor != null)
    return ctor(box)
  ...
  return lightCtor(box,
    {
      onClick = onClickExt
      hotkeys = hotkey == null ? null
        : [ [hotkey, { description = { skip = true }, sound = "click"}] ]
    })
}
...
let function mkMessage(text, customCtor, boxes, style) {
  ...
  //calc omp size with hotkey visible to correct fit
    ...
    children = [
      ...
      hasValidBoxes || !shouldBeBoxes ? null //recalc objects if back scene not loaded yet
        : {
            ...
            onAttach = @() gui_scene.resetTimeout(0.05, @() boxUpdateCount(boxUpdateCount.value + 1))
          }
