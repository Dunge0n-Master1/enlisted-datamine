...
let {groupsList, updateGroupsList, mkGroupListItemName, mkGroupListItemTooltip,
     toggleGroupListItem} = require("editorGroupsControl.nut")
  ...
  return {
    ...
    children = [
      ...
      isTerraforming.value ? @() {
        pos = [hdpx(5), 0]
        flow = FLOW_VERTICAL
        watch = [terraformParams.radius, terraformParams.depth, showPointAction, namePointAction]
        children = [
          { size = [0, hdpx(5)] }
      ...
      isRIToolMode() ? @() {
        watch = [showPointAction, namePointAction]
        pos = [hdpx(5), 0]
        flow = FLOW_VERTICAL
        children = [
          { size = [0, hdpx(5)] }
