...
let function onProjectileHit(evt, eid, comp) {
  let victimEid = evt[0]
  let hitPos = evt[1]
  let hitmarksGroupsId = getHitmarksGroupsId(comp.human_anim__vehicleSelected, @(_, comp) comp.hitmarks__groupsIds.getAll()) ?? []
  let shouldShowHitMarks = getVictimShowHits(victimEid, @(_, comp) comp.deadEntity == null && hitmarksGroupsId.contains(comp.hitmarks_victim__groupId)) ?? false
