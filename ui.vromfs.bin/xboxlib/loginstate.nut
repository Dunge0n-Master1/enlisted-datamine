...
let cachedLoginState = hardPersistWatched("cachedLoginState", false)
...
let function login() {
  logX("Start login")
  let updated = !cachedLoginState.value
  cachedLoginState.update(true)
...
return {
  ...
  subscribe_to_login
  subscribe_to_logout
  is_logged_in = @() cachedLoginState.value
}